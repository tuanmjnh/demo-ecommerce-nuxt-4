...
export default defineEventHandler(async (event) => {
  ensureAuth(event)
  const rs: Common.IResponseItem = { type: 'billing-cancelOrder', message: 'success', status: true, data: null }
  ...
  try {
    ...
    rs.data = await CommonService.update(BillingModel, billingId, {
      $set: { status: 'cancelled', updated: updatedInfo },
      $push: { history: { action: 'cancelled', reason: body.reason, updated: updatedInfo } }
    }, true)
