...
export default defineEventHandler(async (event) => {
  ensureAuth(event)
  const rs: Common.IResponseItems = { type: 'group-distinct', message: 'success', status: true, data: null }

  try {
    const args = getQuery(event)

    const items = await CommonService.distinct(GroupModel, String(args.key), String(args.text || ''))
