...
export default defineEventHandler(async (event) => {
  ensureAuth(event)
  const rs: Common.IResponseItems = { type: 'config-distinct', message: 'success', status: true, data: null }

  try {
    const args = getQuery(event)
    const items = await CommonService.distinct(ConfigModel, String(args.key), String(args.text || ''))
